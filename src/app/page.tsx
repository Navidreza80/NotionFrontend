...
const icons: Record<string, any> = {
  PenSquare, Search, Home, Inbox, Plus, BookOpenText, Star,
  Database, Globe2, Share2, Settings, StoreIcon,
...
const sidebarConfig = {
  ...
  sections: [
    ...
    },
    {
      title: "Shared",
      items: [
        { icon: "Share2", label: "Start collaborating" },
      ],
    },
    {
      title: null,
...
const NavItem = ({ icon: Icon, label, active = false }) => (
  ...
  >
    <Icon size={20} className={active ? "text-white/50" : "text-white/30"} />
  ...
  return (
    <div className="min-h-screen w-full bg-[#191919] text-[#ffffffcf]">
      ...
      <div className="mx-auto w-full gap-0 flex">
        {/* Sidebar */}
        <aside className="sticky top-0 h-screen w-[240px] shrink-0 border-r border-[#383838] bg-[#202020] px-1 pb-4">
          {/* Workspace header */}
          ...
          <div className="px-1 space-y-1">
            <NavItem icon={Share2} label="Start collaborating" />
          </div>

          <SectionTitle></SectionTitle>
          <div className="px-1 space-y-1">
            <NavItem icon={Settings} label="Settings" />
            <NavItem icon={StoreIcon} label="Marketplace" />
            <NavItem icon={Trash2} label="Trash" />
          </div>

          <div className="mt-auto pt-6">
            <Divider />
            <div className="px-2.5 pt-3 text-xs text-zinc-500 flex items-center gap-2">
              <MessageCircle size={14} /> Invite members
            </div>
          </div>
        </aside>
...
/**
 * Small local-only icon aliases to avoid importing extra packages for 
 * Gmail/Slack/etc. These are just visual placeholders.
 */
const StorefrontIcon = (props) => (
  <svg viewBox="0 0 24 24" width="16" height="16" {...props} className="text-zinc-400">
    <path
      fill="currentColor"
      d="M4 4h16l1 5H3l1-5Zm-1 7h18v9H3v-9Zm4 2v5h2v-5H7Zm6 0v5h2v-5h-2Z"
    />
  </svg>
);

const MailIcon = (props) => (
  <svg viewBox="0 0 24 24" width="12" height="12" {...props} className="text-zinc-400">
    <path fill="currentColor" d="M2 6a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6Zm2 0 8 5 8-5H4Z" />
  </svg>
);